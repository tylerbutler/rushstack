...
export class MarkdownDocumenter {
  ...
  protected _writeHeritageTypes(output: DocSection | DocParagraph, apiItem: ApiDeclaredItem): void {
    const configuration: TSDocConfiguration = this._tsdocConfiguration;
  ...
  protected _writeRemarksSection(output: DocSection | DocParagraph, apiItem: ApiItem): void {
    if (apiItem instanceof ApiDocumentedItem) {
      const tsdocComment: DocComment | undefined = apiItem.tsdocComment;
  ...
  protected _writeThrowsSection(output: DocSection | DocParagraph, apiItem: ApiItem): void {
    if (apiItem instanceof ApiDocumentedItem) {
      const tsdocComment: DocComment | undefined = apiItem.tsdocComment;
  ...
  protected _createModifiersCell(apiItem: ApiItem): DocTableCell {
    const configuration: TSDocConfiguration = this._tsdocConfiguration;
  ...
  protected _createPropertyTypeCell(apiItem: ApiItem): DocTableCell {
    const configuration: TSDocConfiguration = this._tsdocConfiguration;
  ...
  protected _writeBetaWarning(output: DocSection | DocParagraph): void {
    const configuration: TSDocConfiguration = this._tsdocConfiguration;
  ...
  protected _appendSection(output: DocSection | DocParagraph, docSection: DocSection): void {
    for (const node of docSection.nodes) {
      output.appendNode(node);
    }
  }

  protected _appendAndMergeSection(output: DocSection, docSection: DocSection): void {
  ...
  protected _deleteOldOutputFiles(): void {
